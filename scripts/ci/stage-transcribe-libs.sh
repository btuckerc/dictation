...
#
# On Linux/Windows, Dictation builds transcribe-cpp in its `dynamic-backends`
...
# them. This script copies them, as a co-located set, next to where the `dictation`
# binary will find them at runtime:
#   - the loader resolves `libtranscribe` via the $ORIGIN-relative rpath baked
#     into `dictation` (see src-tauri/build.rs);
