...
export const ToggleSwitch: React.FC<ToggleSwitchProps> = ({
  ...
  return (
    ...
    >
      <label
        className={`relative flex items-center ${disabled || isUpdating ? "cursor-not-allowed" : "cursor-pointer"}`}
      >
        <input
          type="checkbox"
          aria-label={label}
          value=""
          className="absolute inset-0 w-full h-full opacity-0 z-10 peer cursor-inherit"
          checked={checked}
